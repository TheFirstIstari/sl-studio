...
fn test_database_duplicate_fingerprint() {
    ...
    db.insert_fingerprint("abc123", "/test/path.pdf", "pdf", 1024, "path.pdf").unwrap();
    db.insert_fingerprint("abc123", "/test/path.pdf", "pdf", 1024, "path.pdf").unwrap();
...
fn test_database_get_unprocessed_files() {
    ...
    db.insert_fingerprint("abc123", "/test/path1.pdf", "pdf", 1024, "path1.pdf").unwrap();
    db.insert_fingerprint("def456", "/test/path2.pdf", "pdf", 1024, "path2.pdf").unwrap();
...
fn test_database_checkpoints() {
    ...
    db.checkpoint_update("job-001", "fingerprint123", 10).unwrap();
...
fn test_database_audit_log() {
    ...
    db.log_audit("test_action", "test details", Some(100)).unwrap();
}
