...
impl Default for HardwareStatus {
    fn default() -> Self {
        ...
        let scaling = calculate_scaling(cpu_threads, total_memory_gb, available_memory_gb, &gpu_info);
...
fn calculate_scaling(cpu_threads: u32, total_memory_gb: f64, available_memory_gb: f64, gpus: &[GpuInfo]) -> ResourceScaling {
    ...
    #[cfg(target_os = "windows")]
    {
        if let Some(name) = get_cuda_name() {
            gpus.push(GpuInfo {
                ...
                compute_units: 0,
                supported_backends: vec!["cuda".to_string(), "vulkan".to_string(), "opencl".to_string(), "cpu".to_string()],
            });
        }
    }
